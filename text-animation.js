...
export function initTextAnimations(slide) {
    ...
    // SLIDE NÚMERO 8 Y 9
    if (image) {
        const isLeftOrigin = image.classList.contains('left-origin');
        
        gsap.set(image, { 
            x: isLeftOrigin ? "-110%" : "110%", 
            opacity: 1 
        });
    ...
    // SLIDE NÚMERO 13
    const slide13 = document.querySelector('.slide[data-slide="13"]');
    const cards = slide13.querySelectorAll('.minimalist-card');

    if (cards.length > 0) {
        // Animación para el título
        const title = slide13.querySelector('.minimalist-title');
        gsap.set(title, { y: 30, opacity: 0 });
        
        // Animación para los iconos
        const icons = slide13.querySelectorAll('.card-icon');
        gsap.set(icons, { scale: 0, opacity: 0 });
        
        // Animación para los textos
        const texts = slide13.querySelectorAll('.card-text');
